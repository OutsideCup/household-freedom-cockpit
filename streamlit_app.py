...
base_expenses = st.sidebar.number_input(label="Annual Fixed Bills ($)", value=45000, step=1000)
disc_expenses = st.sidebar.number_input(label="Annual Discretionary ($)", value=25000, step=1000)
...
# Manual input field replaces the simulation toggle
portfolio_value = st.sidebar.number_input(
    label="Enter Current Portfolio Total ($)", 
    value=811006.77, 
    step=1000.0,
    format="%.2f"
)
st.sidebar.caption("⚡ Update this value to sync with your tracker.")
...
with col2:
    with st.container(border=True):
        ...
        st.caption("Manually Synced Value")
